...
![Workflow phase 1](phase-1.png)
     2. **Trigger**：
         - **Configure Variables**: Click the pop-up menu at the bottom of the editing area, select `Define variables`, input `events`, and click the `+` button, creating a variable for event queue. Then input `event` and click the `+` button, which will hold the current event being processed.
...
![Event Loop 3](event-loop-3.png)
     ...
     5. **Final Assembly**:
         - Connect the node groups in approximately the following sequence: 1 -> 2 -> 4 -> 3 using brown Trigger connections.
         - Note: Connect the If block of the forth group to the Let Node of the second group; connect the pink input port of the If Node in the forth group to the output port of the Boolean Input in the first group. The final setup should look like this:
